...
static esp_err_t initialize (){
    ...
    esp_rom_delay_us(MASTER_RESET_PULSE_DURATION);
    //gpio_set_level(pin, 1);
    gpio_set_direction(pin, GPIO_MODE_INPUT);
    esp_rom_delay_us(RECOVERY_DURATION);
    ...
    esp_rom_delay_us(MASTER_RESET_PULSE_DURATION - response_time - presence_time);
    return ESP_OK;
}
...
static uint8_t _read_bit(void)
{
    gpio_set_direction(pin, GPIO_MODE_OUTPUT);
    gpio_set_level(pin, 0);
    esp_rom_delay_us(TIME_SLOT_START_DURATION);
    //gpio_set_level(pin, 1);
    gpio_set_direction(pin, GPIO_MODE_INPUT);
    esp_rom_delay_us(VALID_DATA_DURATION - TIME_SLOT_START_DURATION);
    uint8_t bit = gpio_get_level(pin);
    esp_rom_delay_us(TIME_SLOT_DURATION - RECOVERY_DURATION - VALID_DATA_DURATION);
    return bit;
}
...
static void _send_bit(const uint8_t bit)
{
    ...
    if (bit == 0)
    {
        esp_rom_delay_us(58);//TIME_SLOT_DURATION);
        //gpio_set_level(pin, 1);
        gpio_set_direction(pin, GPIO_MODE_INPUT);
        esp_rom_delay_us(RECOVERY_DURATION);
    }
    else
    {
        ...
        //gpio_set_level(pin, 1);
        gpio_set_direction(pin, GPIO_MODE_INPUT);
        esp_rom_delay_us(TIME_SLOT_DURATION);
    }
}
...
esp_err_t get_temperature(const uint8_t *device, int16_t *temp_x_10){
    rx_t rx = {0};
